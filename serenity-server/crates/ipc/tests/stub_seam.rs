...
fn repository_stub_bin() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .and_then(Path::parent)
        .and_then(Path::parent)
        .expect("repository root")
        .join("output/bin/serenity_worker_stub")
}
...
fn stub_seam_runs_one_job_to_done() {
    let owned_bin = repository_stub_bin();
    let bin = owned_bin.as_path();
    assert!(
        bin.exists(),
        "stub worker binary missing at {} — Phase-A precondition",
        bin.display()
    );
