...
impl ModelFamily {
    ...
    pub(crate) fn worker_binary_name(self) -> &'static str {
        match self {
            ...
            ModelFamily::Flux2 => "serenity_worker_difc",
            ModelFamily::Sensenova => "serenity_worker_sensenova",
            ModelFamily::Krea2 => "serenity_worker_difc",
...
/// Which families the Diffusion Compiler serves in this deployment. Every
...
pub(crate) fn capability_profile_for_model(model: &str) -> JsonValue {
    ...
    if let Ok(family) = model_family(model) {
        ...
        if let Some(obj) = profile.as_object_mut() {
            match compiler_serves(family, model) {
                Ok(()) => {
                    obj.insert("compiler_served".to_string(), JsonValue::Bool(true));
                }
                Err(reason) => {
                    obj.insert("compiler_served".to_string(), JsonValue::Bool(false));
                    obj.insert("production_status".to_string(), JsonValue::String("not_ported".to_string()));
                    obj.insert("reason".to_string(), JsonValue::String(reason));
                }
            }
            obj.insert(
                "schema".to_string(),
