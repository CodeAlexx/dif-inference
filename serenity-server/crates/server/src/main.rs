...
/// Global ComfyUI event fan-out: `(prompt_id, event)` for EVERY job, so the single
/// canvas `/ws` socket (connected at page load, before any job) can translate each
/// WorkerEvent into ComfyUI-format messages. Separate from the per-job `JobChannel`
/// broadcast used by `/v1/progress`; the canvas needs one persistent stream keyed by
/// prompt_id, not a per-job subscription. Bounded like the per-job channel.
pub(crate) type ComfyBus = tokio::sync::broadcast::Sender<(String, WorkerEvent)>;
...
pub(crate) struct JobChannel {
    ...
    /// Optional global ComfyUI fan-out: `(global sender, this job's prompt_id)`.
    /// When set (via `attach_global`), every published event is ALSO forwarded to
    /// the global bus so the canvas `/ws` socket sees it. None keeps behavior
    /// identical to the pre-ComfyUI server.
    global: Mutex<Option<(ComfyBus, String)>>,
}
...
impl JobChannel {
    ...
    /// Wire this per-job channel to the global ComfyUI bus, tagging every future
    /// published event with `prompt_id`. Call right after `new()` so the canvas
    /// `/ws` socket receives the job's progress/done/failed events.
    pub(crate) fn attach_global(&self, bus: ComfyBus, prompt_id: String) {
        *self.global.lock().expect("JobChannel.global poisoned") = Some((bus, prompt_id));
    }
    ...
    fn publish(&self, ev: WorkerEvent) {
        ...
        // Forward to the global ComfyUI bus first (a lagging/no-subscriber send is a
        // harmless Err), so the canvas `/ws` socket sees the same event stream.
        if let Some((bus, prompt_id)) = self
            .global
            .lock()
            .expect("JobChannel.global poisoned")
            .as_ref()
        {
            let _ = bus.send((prompt_id.clone(), ev.clone()));
        }
...
pub(crate) struct AppState {
    ...
    /// CROSS-PATH single-GPU lease (audit L3): the generate driver holds it for
    /// the whole job; the subprocess GPU paths (video/caption/magic) acquire it
    /// or 409. See gpu_lock.rs.
    pub(crate) gpu_owner: gpu_lock::GpuOwner,
    /// The (single) training run launched via /v1/train (Phase D1/D5).
    pub(crate) train: trainer::TrainShared,
...
struct GenerateRequest {
    ...
    #[serde(default)]
    reference_latent_method: Option<String>,
    #[serde(default)]
    reference_latent_count: Option<i64>,
    // ── krea2 FlowEdit (lowered by the Krea2FlowEdit graph node) ──
    #[serde(default)]
    edit_src_image: Option<String>,
    #[serde(default)]
    edit_src_prompt: Option<String>,
    #[serde(default)]
    edit_src_negative: Option<String>,
    #[serde(default)]
    edit_nmax: Option<i64>,
    #[serde(default)]
    edit_nmin: Option<i64>,
    #[serde(default)]
    edit_src_cfg: Option<f64>,
    #[serde(default)]
    edit_auto_mask: Option<bool>,
    #[serde(default)]
    edit_mask_q: Option<f64>,
    #[serde(default)]
    edit_mask_dilate: Option<i64>,
    #[serde(default)]
    edit_mask_warmup: Option<i64>,
...
fn repository_path(relative: &str) -> String {
...
fn local_artifact_manifest(
    ...
    if selected_family == Some(ModelFamily::Chroma) {
        ...
        return Some(LocalArtifactManifest {
            ...
            family: "chroma",
            root,
            production_entry: "serenitymojo/serve/chroma_backend.mojo",
            specs,
        });
    }
    ...
    if m.contains("krea") || selected_family == Some(ModelFamily::Krea2) {
        ...
        return Some(LocalArtifactManifest {
            profile: if is_turbo { "krea2_turbo" } else { "krea2_raw" },
            family: "krea2",
            root,
            production_entry: "serenitymojo/serve/krea2_backend.mojo",
            specs,
        });
    }
    ...
    if m.contains("ideogram") {
        let root = repository_path("models/ideogram4");
        let specs = vec![
            artifact_file("model index", format!("{root}/model_index.json")),
            artifact_file(
                "scheduler config",
            ...
            artifact_file(
                "conditional transformer config",
                format!("{root}/transformer/config.json"),
            ),
            artifact_file(
            ...
            artifact_file(
                ...
                format!("{root}/unconditional_transformer/diffusion_pytorch_model.safetensors"),
            ),
            artifact_file(
                "text encoder config",
                format!("{root}/text_encoder/config.json"),
            ),
            artifact_file(
            ...
            artifact_file(
                ...
                repository_path(
                    "serenitymojo/models/dit/parity/ideogram4_fx_latentnorm.safetensors",
                ),
            ),
        ];
        return Some(LocalArtifactManifest {
    ...
    if m.contains("sensenova") || m.contains("sense_nova") || m.contains("sense-nova") {
        let root = repository_path("models/sensenova-u1");
        let mut specs = vec![
    ...
    if m.contains("hidream") || m.contains("hi-dream") || m.contains("hi_dream") {
        let root = repository_path("models/hidream-o1");
        let mut specs = vec![
...
fn params_from_generate_request(
    ...
    // Omitted steps/cfg take the selected MODEL defaults (the same numbers
    // /v1/capabilities advertises), not the wire struct's frozen 20/4.5.
    // This distinction matters for Krea Raw (52/3.5) versus Turbo (8/0).
    let family_defaults = capabilities::model_family(&params.model).ok();
    if let Some(v) = req.steps {
        params.steps = v;
    } else if let Some(f) = family_defaults {
        params.steps = capabilities::default_steps_for_model(&params.model, f);
    }
    ...
    if let Some(v) = req.reference_image {
        params.reference_image = v;
    }
    if let Some(v) = req.reference_latent_method {
        params.reference_latent_method = v;
    }
    if let Some(v) = req.reference_latent_count {
        params.reference_latent_count = v;
    }
    if let Some(v) = req.edit_src_image {
        params.edit_src_image = v;
    }
    if let Some(v) = req.edit_src_prompt {
        params.edit_src_prompt = v;
    }
    if let Some(v) = req.edit_src_negative {
        params.edit_src_negative = v;
    }
    if let Some(v) = req.edit_nmax {
        params.edit_nmax = v;
    }
    if let Some(v) = req.edit_nmin {
        params.edit_nmin = v;
    }
    if let Some(v) = req.edit_src_cfg {
        params.edit_src_cfg = v;
    }
    if let Some(v) = req.edit_auto_mask {
        params.edit_auto_mask = v;
    }
    if let Some(v) = req.edit_mask_q {
        params.edit_mask_q = v;
    }
    if let Some(v) = req.edit_mask_dilate {
        params.edit_mask_dilate = v;
    }
    if let Some(v) = req.edit_mask_warmup {
        params.edit_mask_warmup = v;
    }
...
fn update_done_record_for_manifest(
    ...
    // Some production workers (currently Krea-2) do not embed the optional PNG
    // genparams tEXt chunk. The queued JobParams already carries the exact
    // lowered request JSON, so preserve that instead of publishing empty or
    // current-UI metadata for a completed artifact.
    e.record.params_json = if params_json.trim().is_empty() {
        e.params.params_json.clone()
    } else {
        params_json
    };
...
fn run_worker_driver(
    ...
    loop {
        ...
        // gpu_wait: a job IS queued but a subprocess path (video/caption/magic)
        // holds the cross-path GPU lease — keep it queued and re-scan shortly
        // (generate never rejects on gpu-busy, it just waits; audit L3).
        let mut gpu_wait = false;
        let promoted = match jobs.lock() {
            Ok(mut book) => match book.iter().position(|e| e.record.state == "queued") {
                Some(i) => {
                    let id = book[i].record.id.clone();
                    match gpu_lock::try_acquire(&gpu_owner, "generate", &id) {
                        Ok(guard) => {
                        ...
                        Err(lease) => {
                            tracing::info!(
                                job = %id, owner_kind = lease.kind, owner_id = %lease.id,
                                "gpu held by subprocess path; job stays queued"
                            );
                            gpu_wait = true;
                            None
                        }
        ...
        let (_gpu_guard, job_id, params, hires_scale, hires_denoise) = match promoted {
            Some(x) => x,
            None if gpu_wait => {
                // Service ctl while the subprocess finishes, then re-scan.
                match ctl.recv_timeout(std::time::Duration::from_millis(250)) {
                    Ok(DriverCtl::Wake) => {}
                    Ok(DriverCtl::Cancel(target)) => {
                        tracing::info!(job = %target, "cancel ignored: not in flight (gpu-wait)");
                    }
        ...
        match outcome {
            JobOutcome::Terminal => {
                ...
                if kind_requires_per_job_recycle(&current_kind) {
                    tracing::info!(
                        %job_id,
                        kind = %current_kind,
                        "recycling worker after terminal job for measured VRAM reclaim"
                    );
                    if let Some(mut finished) = handle.take() {
                        finished.kill();
                    }
                    std::thread::sleep(std::time::Duration::from_millis(800));
                }
    ...
    loop {
        ...
        loop {
            match ctl.try_recv() {
                ...
                Ok(DriverCtl::Wake) => {}
                Ok(DriverCtl::EvictIdle(done)) => {
                    // The GPU lease makes this unreachable for video while a
...
async fn post_generate(
    ...
    match enqueue_generate(&st, req_value) {
        Ok(job_id) => (StatusCode::OK, Json(json!({ "job_id": job_id }))).into_response(),
        Err(resp) => resp,
    }
}

/// Shared enqueue core for BOTH `/v1/generate` and the ComfyUI `/prompt` adapter:
/// lower the workflow (if any), validate, register a per-job channel (wired to the
/// global ComfyUI bus), push the queued JobEntry, and wake the driver. Returns the
/// assigned `job_id` on success, or a ready-to-return error `Response` on rejection
/// (so both callers surface the identical structured errors). See `post_generate`
/// (returns `{job_id}`) and `comfy::post_prompt` (returns `{prompt_id}`).
pub(crate) fn enqueue_generate(
    st: &AppState,
    ...
    // Wire the job's events to the global ComfyUI bus so the canvas `/ws` socket
    // (subscribed once at page load) receives this job's progress/done/failed.
    channel.attach_global(st.comfy_ws.clone(), job_id.clone());
    {
...
fn static_content_type(p: &std::path::Path) -> &'static str {
    match p.extension().and_then(|e| e.to_str()).unwrap_or("") {
        ...
        "svg" => "image/svg+xml",
        "ico" => "image/x-icon",
        // video/audio results must play inline in the browser — octet-stream
        // makes Chrome download instead of play (measured on /out mp4s).
        "mp4" => "video/mp4",
        "webm" => "video/webm",
        "gif" => "image/gif",
        "wav" => "audio/wav",
...
/// GET /v1/gpu — live device readout for the topbar (SerenityUI perf footer
/// parity): name, VRAM used/total, utilization, temperature via nvidia-smi.
async fn get_gpu() -> Json<serde_json::Value> {
    ...
    let Ok(output) = output else {
        return Json(json!({"schema": "serenity.gpu.v1", "available": false, "error": "nvidia-smi not runnable"}));
    };
    ...
    let fields: Vec<String> = text
        ...
        .collect();
    if !output.status.success() || fields.len() < 5 {
        return Json(json!({"schema": "serenity.gpu.v1", "available": false, "error": String::from_utf8_lossy(&output.stderr).trim()}));
    }
    let num = |i: usize| fields[i].parse::<f64>().ok();
    Json(json!({
        "schema": "serenity.gpu.v1",
        "available": true,
        "name": fields[0],
        "vram_used_mib": num(1),
        "vram_total_mib": num(2),
        "utilization_pct": num(3),
        "temperature_c": num(4),
    }))
}
...
fn handle_upload(st: &AppState, body: &str, fallback_stem: &str) -> Response {
    ...
    // JSON arm always lands a `.png` (the canvas sends PNG data URLs) — unchanged.
    write_upload_bytes(st, &bytes, req_name, fallback_stem, "png")
}

/// Land raw image bytes under `<out_dir>/uploads/<stem>-<n>.<ext>` and return the
/// `{name, path, url}` shape both upload arms (JSON base64 + browser multipart)
/// share. The JSON arm always passes `ext="png"`, so its response bytes are
...
fn write_upload_bytes(
    ...
    let n = st.next_id.fetch_add(1, Ordering::Relaxed) + 1;
    let fname = format!("{stem}-{n:04}.{ext}");
...
///   - `multipart/form-data` (browser drag-drop file forms, `curl -F image=@x.png`):
///     the first file field's bytes are saved (extension preserved when a known
///     image type, else `.png`).
///   - anything else: the legacy `{name?, data-base64}` JSON arm (byte-identical to
///     before — the canvas's data-URL upload path is unchanged).
async fn post_upload_image(State(st): State<AppState>, req: axum::extract::Request) -> Response {
    let is_multipart = req
        ...
        .unwrap_or(false);
    if is_multipart {
        return upload_image_multipart(&st, req).await;
    }
    // JSON arm: read the raw body to a String (16 MiB cap), then the shared handler.
...
/// the JSON arm's on-disk layout + response shape (`{name, path, url}`).
async fn upload_image_multipart(st: &AppState, req: axum::extract::Request) -> Response {
    ...
    loop {
        match mp.next_field().await {
            Ok(Some(field)) => {
                ...
                // Take the first field that looks like a file upload: it either
                // carries a filename or is named like the canvas/curl conventions.
                let is_file = !file_name.is_empty()
                    || matches!(
                        field_name.as_str(),
                        "image" | "file" | "data" | "mask" | "upload"
                    );
                if !is_file {
                    continue;
                }
                ...
                let req_name = if !file_name.is_empty() {
                    file_name.as_str()
                } else {
                    field_name.as_str()
                };
                return write_upload_bytes(st, &data, req_name, "init", ext);
            }
            Ok(None) => break,
            Err(e) => {
                return error_detail(
    ...
    error_detail(
        StatusCode::UNPROCESSABLE_ENTITY,
        "no file field found in multipart form",
    )
}
...
fn output_counter_from_name(name: &std::ffi::OsStr) -> Option<u64> {
    let stem = FsPath::new(name).file_stem()?.to_str()?;
    let (_, digits) = stem.rsplit_once('-')?;
    (!digits.is_empty() && digits.bytes().all(|byte| byte.is_ascii_digit()))
        .then(|| digits.parse::<u64>().ok())?
}

/// Recover the shared artifact counter even when jobs.db is absent or stale.
/// Video/train/grid directories and job PNGs live at the output root; uploads
/// are the one counter-backed family stored one level below it. A server restart
/// must never reuse one of those names and truncate a completed artifact.
fn max_existing_output_id(out_dir: &FsPath) -> u64 {
    fn scan(directory: &FsPath, max_seen: &mut u64) {
        let Ok(entries) = std::fs::read_dir(directory) else {
            return;
        };
        for entry in entries.flatten() {
            if let Some(id) = output_counter_from_name(&entry.file_name()) {
...
mod endpoint_tests {
    ...
    fn valid_t2i_params(model: &str) -> JobParams {
        ...
        if normalized.contains("ideogram") {
            // Ideogram-4 admission requires a structured JSON caption object
            // (plain text is off-distribution and rejected pre-queue).
            params.prompt =
                r#"{"high_level_description":"a production gate test prompt"}"#.to_string();
        }
    ...
    fn worker_dispatch_uses_admitted_model_family_classifier() {
        ...
        let cases = [
            ...
            ("klein-4b", "flux2", "serenity_worker_difc"),
    ...
    #[test]
    fn initial_worker_kind_covers_product_workers() {
        assert_eq!(
            kind_from_bin(FsPath::new("/tmp/serenity_worker_krea2")),
            "krea2"
        );
        assert_eq!(
            kind_from_bin(FsPath::new("/tmp/serenity_worker_chroma")),
            "chroma"
        );
        assert_eq!(
    ...
    fn generate_request_accepts_ideogram_prompt_json_bbox_and_logitnormal_scheduler() {
        ...
        // This deployment serves the Diffusion Compiler: Ideogram4 keeps its
        // transport/lowering contract but is refused at the runtime gate.
        let err = validate_generate_runtime_ready(&params, hires_scale).unwrap_err();
        assert!(err.contains("not ported to the Diffusion Compiler"), "{err}");
    }

    #[test]
    fn krea_turbo_omitted_request_fields_use_creator_defaults() {
        let req: GenerateRequest = serde_json::from_value(json!({
            "model": "krea2-turbo",
            "prompt": "a studio bicycle product photograph"
        }))
        .unwrap();
    ...
    #[test]
    fn preflight_report_reflects_qwen_artifact_readiness_and_exposes_block_budget() {
        ...
        assert!(report["error"].as_str().unwrap_or("").contains("not ported to the Diffusion Compiler"));
    ...
    fn preflight_report_describes_vae_override_without_server_side_blocking() {
        ...
        assert!(validate_generate_runtime_ready(&params, 1.0)
            .unwrap_err()
            .contains("not ported to the Diffusion Compiler"));
    }
    ...
    fn workflow_lifted_prequeue_rejections_keep_route_context() {
        ...
        assert_eq!(preflight["capability_profile"]["backend"], "flux2");
        assert_eq!(
    ...
    fn preflight_report_reflects_klein_artifact_readiness_and_keeps_local_block_profile() {
        let params = valid_t2i_params("flux-2-klein-base-9b");
        let report = generate_preflight_report(&params, 1.0);
    ...
    fn capabilities_contract_covers_admitted_features_and_fail_loud_limits() {
        ...
        assert_eq!(zimage_profile["production_status"], "not_ported");
        assert_eq!(
        ...
        assert_eq!(qwen_profile["production_status"], "not_ported");
        ...
        // Unblocked 2026-07-16: flux_backend streams the DiT; the 22.3 GiB
        // whole-resident OOM block no longer applies.
        assert_ne!(flux_profile["production_status"], "blocked");
    }
    ...
    #[test]
    fn restart_counter_reserves_existing_artifact_ids() {
        let root =
            std::env::temp_dir().join(format!("serenity-counter-recovery-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(root.join("video-0042")).unwrap();
        std::fs::write(root.join("job-0017.png"), b"png").unwrap();
        std::fs::create_dir_all(root.join("uploads")).unwrap();
        std::fs::write(root.join("uploads/mask-0049.png"), b"png").unwrap();
        std::fs::write(root.join("refhq_frame240.png"), b"not a counter").unwrap();

        assert_eq!(max_existing_output_id(&root), 49);
        assert_eq!(
            output_counter_from_name(std::ffi::OsStr::new("grid-0102.png")),
            Some(102)
        );
        assert_eq!(
            output_counter_from_name(std::ffi::OsStr::new("refhq_frame240.png")),
            None
        );
        let _ = std::fs::remove_dir_all(root);
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    // Load prior jobs.db rows ONCE (history half of /v1/jobs) and recover the
    // shared counter from both the database and existing artifact names. Video
    // jobs are not rows in jobs.db, so the filesystem half is required to avoid
    // overwriting video-NNNN after a restart.
    let prior = Arc::new(jobs::load_prior_rows(&out_dir.join("jobs.db")));
    let db_id = jobs::max_prior_id(&prior) as u64;
    let artifact_id = max_existing_output_id(&out_dir);
    let next_id = Arc::new(AtomicU64::new(db_id.max(artifact_id)));
    let job_book: Arc<Mutex<Vec<jobs::JobEntry>>> = Arc::new(Mutex::new(Vec::new()));
    tracing::info!(
        prior_rows = prior.len(),
        db_id_base = db_id,
        artifact_id_base = artifact_id,
    ...
    let gpu_owner = gpu_lock::new_owner();
    let gpu_owner_driver = gpu_owner.clone();
    {
        let registry = registry.clone();
    ...
    // Global ComfyUI event bus: the canvas `/ws` socket subscribes; every JobChannel
    // fans its events here. The initial receiver is dropped — sends with no live
    // subscriber are a harmless Err (the canvas connects before it generates).
    let (comfy_ws, _comfy_rx) =
        tokio::sync::broadcast::channel::<(String, WorkerEvent)>(BROADCAST_CAP);
    ...
    let app = Router::new()
        ...
        .route("/v1/train", post(trainer::post_train))
        .route("/v1/train/status", get(trainer::get_train_status))
        .route("/v1/train/sample", post(trainer::post_train_sample))
        ...
        // --- ComfyUI-API compatibility (Tier A) — the Konva canvas speaks ComfyUI;
        //     these thin adapters map onto the existing /v1 engine + graph lowering. ---
        .route("/prompt", post(comfy::post_prompt))
        .route("/ws", get(comfy::get_ws))
        .route("/view", get(comfy::get_view))
        .route("/object_info", get(comfy::get_object_info))
        .route("/object_info/:node", get(comfy::get_object_info_node))
        .route("/system_stats", get(comfy::get_system_stats))
        .route("/queue", get(comfy::get_queue).post(comfy::post_queue))
        .route("/history", get(comfy::get_history))
        .route("/history/:id", get(comfy::get_history_one))
        .route("/interrupt", post(comfy::post_interrupt))
        .route("/models", get(comfy::get_models_comfy))
        .route("/models/loras", get(comfy::get_loras_comfy))
        .route(
            "/models/:mtype/:name",
            axum::routing::delete(models::delete_model),
        )
        .route("/embeddings", get(comfy::get_embeddings))
        // --- Settings / gallery completeness (frontend called these; were 404) ---
        .route("/templates", get(settings::get_templates))
        .route("/folder_paths", get(settings::get_folder_paths))
        .route("/folder_paths/add", post(settings::post_folder_paths_add))
        .route(
            "/stagehand_settings",
            get(settings::get_stagehand_settings).post(settings::post_stagehand_settings),
        )
        .route(
        ...
        .route(
            "/output_files/:name",
            axum::routing::delete(settings::delete_output_file),
        )
        .route(
            "/open_output_dir",
