...
// ── GPU perf footer (SerenityUI parity): live readout in the topbar ──
(function () {
    var el = null;
    function render(gpu) {
        if (!el) el = document.getElementById('topbar-gpu');
        if (!el) return;
        if (!gpu || gpu.available !== true) { el.textContent = 'GPU: n/a'; return; }
        var used = Number(gpu.vram_used_mib) / 1024, total = Number(gpu.vram_total_mib) / 1024;
        var name = String(gpu.name || '').replace(/^NVIDIA\s+/, '');
        el.textContent = name + ' \u00b7 ' + used.toFixed(1) + '/' + total.toFixed(1) + ' GB \u00b7 ' +
            Math.round(Number(gpu.utilization_pct) || 0) + '% \u00b7 ' + Math.round(Number(gpu.temperature_c) || 0) + '\u00b0C';
    }
    function poll() {
        fetch('/v1/gpu', { cache: 'no-store' }).then(function (r) { return r.json(); }).then(render).catch(function () { render(null); });
    }
