...
var GenerateTab = (function () {
    ...
    function buildGenerateLeftHTML() {
        ...
        var dynamicThresholdBody =
            ...
            '</div>';
        function gridAxisSelect(id, allowNone) {
            return '<select id="' + id + '" class="gen-select">' + (allowNone ? '<option value="">none</option>' : '') +
                ['seed', 'cfg', 'steps', 'sampler', 'scheduler'].map(function (a) { return '<option value="' + a + '">' + a + '</option>'; }).join('') + '</select>';
        }
        var gridBody =
            '<div class="gen-capability-note">Values are comma-separated. Numeric axes take numbers, sampler/scheduler take names. Each cell is a normal generate job with the current parameters.</div>' +
            '<div class="gen-setting-row gen-param-row" data-param-search="grid x axis"><label class="gen-setting-label" for="gen-grid-x-axis">X axis</label>' + gridAxisSelect('gen-grid-x-axis', false) +
            '<input id="gen-grid-x-values" type="text" class="gen-text-input" placeholder="1,3,5,7" value="1,3,5,7"></div>' +
            '<div class="gen-setting-row gen-param-row" data-param-search="grid y axis"><label class="gen-setting-label" for="gen-grid-y-axis">Y axis</label>' + gridAxisSelect('gen-grid-y-axis', true) +
            '<input id="gen-grid-y-values" type="text" class="gen-text-input" placeholder="optional"></div>' +
            '<div class="gen-setting-row gen-param-row" data-param-search="grid z axis"><label class="gen-setting-label" for="gen-grid-z-axis">Z axis</label>' + gridAxisSelect('gen-grid-z-axis', true) +
            '<input id="gen-grid-z-values" type="text" class="gen-text-input" placeholder="optional (one page per value)"></div>' +
            '<div class="gen-setting-row gen-param-row"><button id="gen-grid-run" class="gen-action-btn gen-action-btn-wide">Generate Grid</button></div>' +
            '<div id="gen-grid-status" class="gen-capability-note"></div>';
        ...
        return '<div class="gen-workspace-parameter-head"><div><strong>Parameters</strong><span id="gen-arch-badge" class="gen-arch-badge">IMAGE</span></div>' +
            ...
            generateGroup('gen-grid-header', 'Grid (XYZ sweep)', gridBody, false, 'Sweep up to three axes (seed, cfg, steps, sampler, scheduler); one job per cell, composited into a labelled grid via /v1/grid.') +
    ...
    function buildGeneratePromptHTML() {
        return '<div class="gen-workspace-prompts"><div class="gen-workspace-prompt-main">' +
            ...
            '<textarea id="gen-prompt" class="gen-textarea gen-workspace-prompt-textarea" rows="3" placeholder="Describe the image you want... (text:1.3) <lora:name:0.8> <random:a|b|c>"></textarea>' +
            '<div id="gen-prompt-syntax-note" class="gen-capability-note" style="display:none"></div>' +
    ...
    function buildGenerateLibraryHTML() {
        return '<div class="gen-workspace-library-tabs">' +
            ...
            '<span id="gen-run-timer" class="gen-run-timer" title="Wall time from Generate to the finished image/video">Time : \u2014</span>' +
    ...
    function cacheElements() {
        ...
        var gridRun = document.getElementById('gen-grid-run');
        if (gridRun)
            gridRun.addEventListener('click', runGridSweep);
    ...
    function updateMinimaxH3VideoUI(arch, enteringH3) {
        ...
        // Default to the server's first supported profile (the Diffusion
        // Compiler's admitted 832x480 / 5.17 s geometry) when entering H3.
        var defaultProfile = runner && Array.isArray(runner.supported_profiles)
            ? runner.supported_profiles[0] : null;
        if (enteringH3 && defaultProfile && Number(defaultProfile.width) > 0) {
            state.width = Number(defaultProfile.width);
            state.height = Number(defaultProfile.height);
            state.fps = Number(defaultProfile.fps) || 24;
            state.seconds = Number(defaultProfile.duration) ||
                (Number(defaultProfile.frames) / state.fps);
        }
        var durationMax = minimaxH3MaxDurationForSize(state.width, state.height);
        ...
        if (runtimeLabel)
            runtimeLabel.textContent = runnerReady
                ? 'MiniMax-H3 · Diffusion Compiler · ready'
                : 'MiniMax-H3 · Diffusion Compiler · unavailable';
        setPreviewModelBadges(state.model, arch);
        updateAspectPreview();
        updateDurationHint();
        updateAdvancedSamplingUI(null);
    ...
    function updateGenerateUIForArch(arch) {
        ...
        var runtimeLabel = document.getElementById('gen-runtime-label');
        if (runtimeLabel)
            runtimeLabel.textContent = profile.backend
                + (profile.engine === 'diffusion-compiler' ? ' · Diffusion Compiler' : ' · Mojo image worker')
                + ' · ' + (profile.production_status || 'admitted')
                + (profile.production_status === 'not_ported' && profile.reason ? ' — ' + profile.reason : '');
        var aspects = getActiveAspects();
        if (els.aspectDropdown) {
            els.aspectDropdown.innerHTML = buildAspectOptions();
        }
    ...
    function buildGenerateRequest(seed) {
        ...
        // SerenityUI prompt-syntax parity: (w:1.3) pass-through, <lora:> extraction,
        // <random:> seeded by THIS job's seed; the raw text rides along as prompt_raw.
        var syntax = (typeof SerenityPromptSyntax !== 'undefined')
            ? SerenityPromptSyntax.resolve(finalPrompt, seed)
            : { resolved: finalPrompt, loras: [], notes: [], had_syntax: false };
        showPromptSyntaxNotes(syntax.notes);
        var request = {
            model: state.model || '',
            prompt: syntax.resolved,
            width: state.width,
            height: state.height,
            steps: state.steps,
            seed: seed,
        ...
        if (syntax.had_syntax)
            request.prompt_raw = finalPrompt;
        var enabledLoras = state.loras.filter(function (lora) { return lora.enabled !== false; })
            .map(function (lora) { return { name: lora.name, weight: Number(lora.strength) }; });
        if (syntax.loras.length && typeof SerenityPromptSyntax !== 'undefined')
            enabledLoras = SerenityPromptSyntax.mergeLoras(enabledLoras, syntax.loras, syntax.notes);
        if (enabledLoras.length)
            request.lora = enabledLoras;
        return request;
    }
    function parseGridValues(axis, text) {
        var parts = String(text || '').split(',').map(function (t) { return t.trim(); }).filter(Boolean);
        if (axis === 'sampler' || axis === 'scheduler')
            return parts;
        return parts.map(Number).filter(function (v) { return Number.isFinite(v); });
    }
    function runGridSweep() {
        var status = document.getElementById('gen-grid-status');
        var say = function (t) { if (status) status.textContent = t; };
        var seed = state.seed === -1 ? Math.floor(Math.random() * 4294967296) : state.seed;
        var body = buildGenerateRequest(seed);
        delete body.images;
        var used = {};
        var dims = ['x', 'y', 'z'];
        for (var d = 0; d < dims.length; d++) {
            var axisEl = document.getElementById('gen-grid-' + dims[d] + '-axis');
            var axis = axisEl ? axisEl.value : '';
            if (!axis) continue;
            if (used[axis]) { say('The same axis may not be used on two dimensions: ' + axis); return; }
            var values = parseGridValues(axis, (document.getElementById('gen-grid-' + dims[d] + '-values') || {}).value);
            if (!values.length) { say('No usable values for ' + dims[d].toUpperCase() + ' axis ' + axis); return; }
            used[axis] = true;
            body[dims[d] + '_axis'] = axis;
            body[dims[d] + '_values'] = values;
            delete body[axis];
        }
        if (!body.x_axis) { say('X axis is required'); return; }
        say('Grid running… one job per cell; watch the Queue tab.');
        var t0 = Date.now();
        fetch('/v1/grid', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
            .then(function (r) { return r.json().then(function (j) { return { ok: r.ok, status: r.status, json: j }; }); })
            .then(function (res) {
                if (!res.ok) { say('Grid failed (' + res.status + '): ' + (res.json && (res.json.error || JSON.stringify(res.json)))); return; }
                var j = res.json;
                var paths = j.paths || [j.path];
                var names = paths.map(function (p) { return String(p).split('/').pop(); });
                say(j.grid_id + ' done in ' + Math.round((Date.now() - t0) / 1000) + 's · ' + (j.cells || []).length + ' cells · ' + names.join(', '));
                if (els.previewImg && names[0]) {
                    els.previewImg.src = '/out/' + encodeURIComponent(names[0]) + '?t=' + Date.now();
                    els.previewImg.style.display = 'block';
                }
                if (typeof refreshHistory === 'function') refreshHistory();
            })
            .catch(function (e) { say('Grid request failed: ' + e.message); });
    }
    function showPromptSyntaxNotes(notes) {
        var el = document.getElementById('gen-prompt-syntax-note');
        if (!el)
            return;
        el.textContent = notes && notes.length ? 'Prompt syntax: ' + notes.join('; ') : '';
        el.style.display = notes && notes.length ? '' : 'none';
    }
    ...
    // Run timer: wall time from the Generate click to the finished artifact (image or video,
    // whole batch). Shown after the LoRAs library tab; frozen at the final total when done.
    var runTimer = { start: 0, handle: null, lastTotalMs: 0 };
    function formatRunTime(ms) {
        ...
        var rest = s - m * 60;
        return m + ' min ' + rest.toFixed(1) + ' s (' + s.toFixed(1) + ' s)';
    }
    function renderRunTimer(final) {
        var el = document.getElementById('gen-run-timer');
        if (!el)
            return;
        var ms = runTimer.start ? (Date.now() - runTimer.start) : runTimer.lastTotalMs;
        el.textContent = 'Time : ' + (ms > 0 ? formatRunTime(ms) : '\u2014');
        el.classList.toggle('running', !final && !!runTimer.start);
    }
    function startRunTimer() {
        if (runTimer.handle)
            clearInterval(runTimer.handle);
        runTimer.start = Date.now();
        runTimer.handle = setInterval(function () { renderRunTimer(false); }, 100);
        renderRunTimer(false);
    }
    function stopRunTimer() {
        if (!runTimer.start)
            return;
        runTimer.lastTotalMs = Date.now() - runTimer.start;
        runTimer.start = 0;
        if (runTimer.handle)
            clearInterval(runTimer.handle);
        runTimer.handle = null;
        renderRunTimer(true);
    }
    ...
    function createThumb(item, galleryIndex) {
        ...
        wrap.querySelector('.gen-thumb-star').addEventListener('click', function (e) {
            ...
            var serverId = galleryServerId(state.gallery[idx]);
            if (serverId) {
                fetch('/v1/gallery/' + encodeURIComponent(serverId) + '/favorite', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ favorite: state.gallery[idx].starred })
                }).catch(function () {});
            }
            renderGallery();
        });
    ...
    /** Server gallery id for a history item: the artifact basename (job-0007, grid-0012). */
    function galleryServerId(item) {
        var src = String(item && item.src || '');
        var name = src.split('?')[0].split('/').pop();
        var m = /^((?:job|grid)-\d+)\.png$/i.exec(name || '');
        return m ? m[1] : '';
    }
    ...
    function refreshHistory() {
        fetch('/v1/history/artifacts', { cache: 'no-store' })
            ...
            // SerenityUI parity: stars persist server-side (gallery favorites),
            // not only in this page's memory.
            return fetch('/v1/gallery', { cache: 'no-store' })
                .then(function (response) { return response.ok ? response.json() : null; })
                .then(function (gallery) {
                    var favorites = {};
                    var list = gallery && Array.isArray(gallery.items) ? gallery.items : [];
                    list.forEach(function (entry) {
                        if (entry && entry.favorite === true && entry.id)
                            favorites[String(entry.id)] = true;
                    });
                    state.gallery.forEach(function (item) {
                        var id = galleryServerId(item);
                        if (id && favorites[id])
                            item.starred = true;
                    });
                    return state.gallery;
                })
                .catch(function () { return state.gallery; });
        })
            .then(function (items) {
